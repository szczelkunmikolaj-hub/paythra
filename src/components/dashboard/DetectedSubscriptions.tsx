  ...
  return (
    <Card className="border-primary/20 shadow-card">
      ...
      <CardContent className="space-y-3">
        {detected.map((d) => (
          <div key={d.merchant} className="flex items-center justify-between rounded-lg border border-border bg-muted/50 p-3">
            <div>
              <p className="font-medium text-foreground">
                Possible subscription: {d.merchant} €{d.amount.toFixed(2)}/{d.cycle === "monthly" ? "mo" : "yr"}
              </p>
              <p className="text-xs text-muted-foreground">Found {d.occurrences} matching transactions</p>
            </div>
